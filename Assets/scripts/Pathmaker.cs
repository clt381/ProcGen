...
public class Pathmaker : MonoBehaviour {
    ...
    //public static List<GameObject> floorList = new List<GameObject>();     //should be static because it will keep track of all pathmakers
    ...
    void Start()
    {
        ...
        numberOfFloors = Random.Range(300, 800);
    }


	void Update () {

        if (fController.counter < numberOfFloors)
        {
            ranNumber = Random.Range(0.0f, 1.0f);
            ranNumber2 = Random.Range(0.0f, 1.0f);

            if (ranNumber < 0.25f)
            {
                transform.Rotate(0, 90f, 0);
            }
            else if (ranNumber > 0.25f && ranNumber < 0.5f)
            {
                transform.Rotate(0, -90f, 0);
            }
            else if (ranNumber > 0.95f && ranNumber < 1.0f )
            {
            ...
            if (ranNumber < 0.1)
            {
                var f = Instantiate(floorPrefab1, transform.position, transform.rotation);
                fController.floorList.Add(f.gameObject);
            }
            else if (ranNumber > 0.1 && ranNumber < 0.2)
            {
                var f = Instantiate(floorPrefab2, transform.position, transform.rotation);
                fController.floorList.Add(f.gameObject);
            }
            else if (ranNumber > 0.2 && ranNumber < 0.6)
            {
                if (ranNumber2 < 0.5)
                {
                    var f = Instantiate(floorPrefab3, transform.position, transform.rotation);
                    fController.floorList.Add(f.gameObject);
                }
                else if (ranNumber2 > 0.5)
                {
                    var f = Instantiate(floorPrefab4, transform.position, transform.rotation);
                    fController.floorList.Add(f.gameObject);
                }
            }
            else if (ranNumber > 0.6)
            {
                var f = Instantiate(floorPrefab5, transform.position, transform.rotation);
                fController.floorList.Add(f.gameObject);
            }
            //var f = Instantiate(floorPrefab, transform.position, transform.rotation);
